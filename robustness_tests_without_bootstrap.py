...
Run
---
  python3 robustness_tests.py

Assumes Data/prices_daily.csv exists.
"""
...
def slice_and_clean(log_ret, start, end,
    ...
    if dropped:
        print(f"  [clean] Dropped {len(dropped)} sparse: "
              f"{sorted(dropped)}")
...
def equal_corr(ret, freq=12):
    """
    Equal-correlation shrinkage.
    Shrinks sample correlation matrix toward average
    off-diagonal correlation, then reconstructs covariance.
    """
    mu      = ret.mean().values * freq
    S       = ret.cov().values
...
def jobson_korkie_test(ret_pre, ret_post,
    ...
    Tests whether the observed Sharpe ratio improvement is
    statistically significant or could be due to sampling noise.
    """
    ...
    # Memmel (2003) standard error — accounts for
    # estimation uncertainty in both Sharpe ratios
    se = np.sqrt(
        (1 / T1) * (1 + 0.5 * sr1**2) +
        (1 / T2) * (1 + 0.5 * sr2**2)
    )
    ...
    z = (sr2 - sr1) / se
    p = 1 - stats.norm.cdf(z)   # one-sided p-value
...
def box_m_test(ret_pre, ret_post):
    ...
    H1: Sigma_pre != Sigma_post

    Uses chi-squared approximation.
    A significant result means the covariance structure
    changed between periods — direct evidence of structural
    transformation in the correlation/risk structure.
    """
    n1, p = ret_pre.shape
    n2, _ = ret_post.shape
...
def plot_estimator_comparison(ret_pre, ret_post,
    ...
    """
    Plots frontiers under three covariance estimators:
      - Sample covariance (no shrinkage)
      - Equal-correlation shrinkage
      - Ledoit-Wolf shrinkage (primary)

    If all three show the same directional shift, estimation
    error is not driving the result.
    """
    PRE_C  = "#1D9E75"
    POST_C = "#534AB7"
    ...
    for ax, ret, rf, title, col_idx in [
        (axes[0], ret_pre,  rf_pre,
         "Pre-COVID (Jan 2015\u2013Dec 2019)", 0),
        (axes[1], ret_post, rf_post,
         "Post-COVID (Jan 2020\u2013Dec 2024)", 1),
    ]:
...
def transaction_cost_analysis(ret_pre, ret_post,
    ...
    period-specific India transaction costs.

    Pre-COVID costs are higher (traditional brokers dominated).
    Post-COVID costs are lower (discount brokers mainstream).

    This means the fixed-cost assumption in earlier analysis
    was conservative — real net advantage is likely larger.
    """
    ...
    # One-way turnover between the two tangency portfolios
...
def sector_weight_check(ret_pre, ret_post, out_path):
    """
    Left chart: universe sector weights (equal-weight)
                vs actual Nifty 50 index weights.
                Checks whether stock selection is representative.

    Right chart: tangency portfolio sector weights pre vs post.
                 Shows sectoral rotation in the optimal portfolio.
    """
    tickers = list(ret_pre.columns)

    # Universe sector weights (equal-weight)
    sector_counts = {}
    for t in tickers:
...
def banking_decomposition(ret_pre, ret_post, out_path):
    """
    Splits banking sector into:
      Retail banks    — HDFC, Kotak, Axis, ICICI, SBI
      Wholesale banks — PNB, Bank of Baroda, IndusInd

    Left chart:  individual bank returns pre vs post
    Right chart: average return by sub-sector pre vs post

    Addresses reviewer feedback on proprietary trading
    and credit split — shows whether underperformance
    is uniform or concentrated in a specific bank type.
    """
    ...
    # Add retail/wholesale sub-type labels
...
def main():
    ...
    # Compute Ledoit-Wolf estimates and tangency weights
    # needed for the Jobson-Korkie test
    ...
    # Save all key numbers to one CSV
    summary = {
